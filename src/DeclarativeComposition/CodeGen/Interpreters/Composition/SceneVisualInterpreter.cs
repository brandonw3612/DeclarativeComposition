...
namespace DeclarativeComposition.CodeGen.Interpreters.Composition;

public class SceneVisualInterpreter : ObjectInterpreter
{
    ...
    public override string Interpret(ExpressionNode value, Context context)
    {
        ...
        if (oValue.Name is null)
            context.InitializerBody.Add($"var {localName} = Microsoft.UI.Composition.Scenes.SceneVisual.Create(_compositor);");
        else
        {
            ...
            context.InitializerBody.Add($"{localName} = Microsoft.UI.Composition.Scenes.SceneVisual.Create(_compositor);");
        }
