...
public class RedirectVisualTest() : TestBase("ProviderTests/RedirectVisualTest")
{
    protected override void VerifyAst(RootNode root)
    {
        ...
        Assert.Equal("RedirectVisual", firstChild.Type);
        ...
        Assert.Equal("RedirectVisual", (firstChild.Properties[0].Value as StringLiteralNode)?.Content);
        ...
        Assert.Equal("source", firstChild.Properties[22].Name);
        Assert.Equal("_compositor.CreateSpriteVisual()", (firstChild.Properties[22].Value as SharpCodeNode)?.Code);
    }
}